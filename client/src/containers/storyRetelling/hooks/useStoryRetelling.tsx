...
export const useStoryRetelling = (client: RecordingServiceClient, tasks: Task.AsObject[], index: number, incrementStep: ()=>void, setDoneTaskId: Dispatch<SetStateAction<number>>) => {
  ...
  useEffect(() => {
    ...
    // 音声提示前のカウントダウンタイマー
    setTimerBeforeStarting(setInterval(() => {
  ...
  useEffect(() => {
    if (isSkipRender) return
    setPercent((task.msBeforeStarting/1000-refCountBeforeStarting.current) / (task.msBeforeStarting/1000) * 100)
